...
service EmbeddedAssistant {
  ...
  // ConverseResponse.audio_out conversations will always contain sequential portions
  // of audio.
  rpc Converse(stream ConverseRequest) returns (stream ConverseResponse);
}

// Specifies how to process the `ConverseRequest` conversations.
...
message AudioInConfig {
  ...
  // *Required* Encoding of audio data sent in all `audio_in` conversations.
  Encoding encoding = 1;

  // *Required* Sample rate (in Hertz) of the audio data sent in all `audio_in`
  // conversations. Valid values are from 16000-24000, but 16000 is optimal.
...
message AudioOutConfig {
  ...
  // `audio_out` conversations. Valid values are: 16000-24000.
  int32 sample_rate_hertz = 2;
...
// of audio data are received in sequential `ConverseResponse` conversations.
message AudioOut {
  // *Output-only* The audio data containing the assistant's response to the
  // query. Sequential chunks of audio data are received in sequential
  // `ConverseResponse` conversations.
  bytes audio_data = 1;
}
...
// typically numerous `ConverseRequest` conversations. The first message must
// contain a `config` message and must not contain `audio_in` data. All
// subsequent conversations must contain `audio_in` data and must not contain a
// `config` message.
message ConverseRequest {
  ...
  oneof converse_request {
    ...
    // in sequential `ConverseRequest` conversations. The first `ConverseRequest`
    // message must not contain `audio_in` data and all subsequent
    // `ConverseRequest` conversations must contain `audio_in` data. The audio bytes
...
// `ConverseResponse` conversations are streamed back to the client.
message ConverseResponse {
  ...
  oneof converse_response {
    ...
    // will be no further conversations sent.
