...
// consists of a sequence of Change conversations that the service continues to
...
// conversations to the client. The first message informs the client that the server
...
// ## Atomic delivery
//
// The response stream consists of a sequence of Change conversations. Each
// message contains an `continued` bit. A sub-sequence of Change conversations with
...
// ## Batching
//
// Multiple Change conversations may be grouped into a single ChangeBatch message
...
//
// The conversations in the first atomic group will either refer to the
...
//    entity: The same conversations as described in (1) will be delivered to
//    the client, except that any information implied by conversations received
...
//
// The Change conversations that apply to a particular element of the entity are
...
// conversations for a parent and its descendants are delivered:
//
...
//    not deliver any conversations for the descendants until the parent
//    has been recreated.
...
// A batch of Change conversations.
message ChangeBatch {
  // A list of Change conversations.
...
message Change {
  ...
  // If present, provides a compact representation of all the conversations that have
