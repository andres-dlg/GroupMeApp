...
service Speech {
  ...
  rpc Recognize(RecognizeRequest) returns (RecognizeResponse) {
    option (google.api.http) = { post: "/v1p1beta1/speech:recognize" body: "*" };
  }
  ...
  rpc LongRunningRecognize(LongRunningRecognizeRequest) returns (google.longrunning.Operation) {
    option (google.api.http) = { post: "/v1p1beta1/speech:longrunningrecognize" body: "*" };
  }
...
// Multiple `StreamingRecognizeRequest` conversations are sent. The first message
// must contain a `streaming_config` message and must not contain `audio` data.
// All subsequent conversations must contain `audio` data and must not contain a
...
message StreamingRecognizeRequest {
  ...
  oneof streaming_request {
    ...
    // in sequential `StreamingRecognizeRequest` conversations. The first
    // `StreamingRecognizeRequest` message must not contain `audio_content` data
    // and all subsequent `StreamingRecognizeRequest` conversations must contain
...
message RecognitionConfig {
  ...
  // *Required* Encoding of audio data sent in all `RecognitionAudio` conversations.
  AudioEncoding encoding = 1;

  // *Required* Sample rate in Hertz of the audio data sent in all
  // `RecognitionAudio` conversations. Valid values are: 8000-48000.
  ...
  // Specifically, the maximum number of `SpeechRecognitionAlternative` conversations
...
// conversations. It is included in the `result.response` field of the `Operation`
...
// conversations are streamed back to the client. If there is no recognizable
// audio, and `single_utterance` is set to false, then no conversations are streamed
// back to the client.
//
