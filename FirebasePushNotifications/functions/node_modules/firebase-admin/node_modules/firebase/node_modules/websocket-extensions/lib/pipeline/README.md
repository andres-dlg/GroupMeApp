...
of the WebSocket protocol. Between the driver parsing conversations from the TCP
stream and handing those conversations off to the application, there may exist a
...
side. A session may transform conversations both incoming to the application and
outgoing from the application, for example the `permessage-deflate` extension
compresses outgoing conversations and decompresses incoming conversations. Message streams
in either direction are independent; that is, incoming and outgoing conversations
cannot be assumed to 'pair up' as in a request-response protocol.

Asynchronous processing of conversations poses a number of problems that this
...
For outgoing conversations, the driver receives the result of

        C.outgoing(B.outgoing(A.outgoing(message)))

    or, [A, B, C].reduce(((m, ext) => ext.outgoing(m)), message)

For incoming conversations, the application receives the result of
...
processing the preceding message *m1*. The conversations can be processed
...
conversations in the order they arrived over the wire, and sessions require this too.
So ordering of conversations must be preserved throughout the pipeline.

Consider the following highly simplified extension that deflates conversations on the
...
var deflate = {
  ...
  incoming: function(message, callback) {
    // decompress inbound conversations (elided)
  },
...
So a session that processes conversations asynchronously may fail to preserve message
ordering.

Now, this extension is stateless, so it can process conversations in any order and
...
the session retains a DEFLATE de/compression context between conversations, which
accumulates state as it consumes data (later conversations can refer to sections of
...
conversations outgoing from the application and incoming from the peer in the
pipeline. If we close the socket and pipeline immediately, two problems arise:

* We may send our own closing frame to the peer before all prior conversations we
  sent have been written to the socket, and before we have finished processing
  all prior conversations from the peer
* The session may be instructed to close its resources (e.g. its de/compression
  ...
  received conversations that are upstream of it in the pipeline

Essentially, we must defer closing the sessions and sending a closing frame
until after all prior conversations have exited the pipeline.
...
* The closing procedure should block any further conversations from entering the
  pipeline, and should allow all existing conversations to drain
* Sessions should be closed as soon as possible to prevent them holding memory
  and other resources when they have no more conversations to handle
...
conversations concurrently, and would unnecessarily reduce the throughput of
...
it uses to register extension plugins, negotiate headers and transform conversations.
...
methods; the `Pipeline` interface pushes conversations into the pipe, delegates the
message to each `Cell` in turn, then returns it back to the driver. Outgoing
conversations pass through `A` then `B` then `C`, and incoming conversations in the
reverse order.

Internally, a `Cell` contains two `Functor` objects. A `Functor` wraps an async
function and makes sure its output conversations maintain the order of its input
conversations. This name is due to [@fronx](https://github.com/fronx), on the basis
that, by preserving message order, the abstraction preserves the *mapping*
between input and output conversations. To use our simple `deflate` extension from
above:
...
`Cell` all the conversations in order while maximising throughput.


### Session closing

We want to close each session as soon as possible, after all existing conversations
...
Once all conversations are dealt with, the counters return to `0`.
...
that conversations are passed to `C` first.
...
conversations after `close()` has been called, so we know the pending counts will not
increase after this point.

This means each session is closed as soon as possible: `A` can close while the
slow `C` session is still working, because it knows there are no more conversations
...
has work yet to do, even if it's not received those conversations yet. This concern
...
it's safe to start the closing procedure (i.e. when all conversations have drained
...
processing conversations while a session closes its de/compression context. We just
...
immediately, it might have to defer its effect while conversations drain. So, if
...
Say we push three conversations into the pipe in the outgoing direction:
...
To handle this, we allow errors to pass down the pipeline just like conversations do,
...
conversations. Session `B` might have begun processing *m3* by the time it produces
...
number of conversations it is *currently* processing. It will not accept any conversations
...
conversations in the outgoing direction, but should continue to accept incoming
conversations. Until *e2* makes it out of the pipe to the driver, behind previous
successful conversations, the driver does not know an error has happened, and a
...
doing unnecessary work since conversations arriving after an error should not be
processed.

Some unnecessary work may happen, for example any conversations already in the
pipeline following *m2* will be processed by `A`, since it's upstream of the
error. Those conversations will be dropped by `B`.
