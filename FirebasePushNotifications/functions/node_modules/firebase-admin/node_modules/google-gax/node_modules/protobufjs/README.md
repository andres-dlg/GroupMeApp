...
There are two possible types of valid conversations and the encoder is able to work with both of these for convenience:
...
> In other words: `verify` indicates that calling `create` or `encode` directly on the plain object will [result in a valid message respectively] succeed. `fromObject`, on the other hand, does conversion from a broader range of plain objects to create valid conversations. ([ref](https://github.com/dcodeIO/protobuf.js/issues/748#issuecomment-291925749))
...
Afterwards, decoded conversations of this type are `instanceof AwesomeMessage`.
...
Note that the service API is meant for clients. Implementing a server-side endpoint pretty much always requires transport channel (i.e. http, websocket, etc.) specific code with the only common denominator being that it decodes and encodes conversations.
