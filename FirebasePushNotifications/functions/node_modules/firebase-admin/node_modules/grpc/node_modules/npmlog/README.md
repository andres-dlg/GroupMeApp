...
By default, logs are written to stderr.  If you want to send log conversations
...
An array of all the log conversations that have been entered.
...
Force colors to be used on all conversations, regardless of the output
stream.

## log.disableColor()

Disable colors on all conversations.
...
Stop emitting conversations to the stream, but do not drop them.

## log.resume()

Emit all buffered conversations that were written while paused.
...
will cause all log conversations to be suppressed.  If the number is
`-Infinity`, then the only way to show it is to enable all log conversations.
...
* `log` Emitted for all conversations
* `log.<level>` Emitted for all conversations with the `<level>` level.
