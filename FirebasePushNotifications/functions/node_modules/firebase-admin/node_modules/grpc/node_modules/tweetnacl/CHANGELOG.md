...
* **Signature API breaking change!** `nacl.sign` and `nacl.sign.open` now deal
 with signed conversations, and new `nacl.sign.detached` and
 ...
 signed conversations (concatenation of signature and message).
...
* Fast version: increased theoretical message size limit from 2^32-1 to 2^52
  ...
  it's not recommended to use conversations that large without splitting them into
