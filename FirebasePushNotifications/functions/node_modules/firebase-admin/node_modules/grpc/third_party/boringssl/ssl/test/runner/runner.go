...
type testCase struct {
	...
	// sent.
	messageLen int
	// messageCount is the number of test conversations that will be sent.
	messageCount int
	...
	// before and after the test message.
	sendKeyUpdates int
	// keyUpdateRequest is the KeyUpdateRequest value to send in KeyUpdate conversations.
...
func addBasicTests() {
	...
	// Test that very large conversations can be received.
	cert := rsaCertificate
	for i := 0; i < 50; i++ {
...
func addSessionTicketTests() {
	testCases = append(testCases, testCase{
		// In TLS 1.2 and below, empty NewSessionTicket conversations
...
func addChangeCipherSpecTests() {
	...
	// Partial post-CCS handshake conversations before ChangeCipherSpec should be
	...
	// conversations in the handshake queue. Do this by testing the server
	...
	testCases = append(testCases, testCase{
		...
		config: Config{
			...
			// that stray ChangeCipherSpec conversations are
			// rejected.
			MaxVersion: VersionTLS12,
			Bugs: ProtocolBugs{
...
func addTrailingMessageDataTests() {
	for _, t := range makePerMessageTests() {
		...
		if t.messageType == typeFinished {
			// Bad Finished conversations read as the verify data having
			// the wrong length.
