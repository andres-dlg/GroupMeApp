...
failure conversations, propagate fatal failures, reuse and speed up your
...
user-visible output. However, we may add `SUCCEED()` conversations to Google Test's
...
You can then use the `<<` operator to stream conversations to the
`AssertionResult` object.

To provide more readable conversations in Boolean assertions
...
If you want informative conversations in `EXPECT_FALSE` and `ASSERT_FALSE`
...
extra logging or custom failure conversations, but that usually clutters up
...
could result in conversations like these:
...
Some tips on using `SCOPED_TRACE`:
  ...
  1. You can use `SCOPED_TRACE` in an inner scope when there is one in the outer scope. In this case, all active trace points will be included in the failure conversations, in reverse order they are encountered.
