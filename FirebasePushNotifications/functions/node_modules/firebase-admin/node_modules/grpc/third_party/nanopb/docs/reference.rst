...
PB_FIELD_32BIT                 Add support for tag numbers > 65535 and fields
                               ...
                               Compiler error will tell if you need this.
PB_NO_ERRMSG                   Disables the support for error conversations; only
...
raising some datatype limits to suit larger conversations. Their need is recognized
...
* The field name pattern is matched against a string of form *'Message.field'*.
  For nested conversations, the string is *'Message.SubMessage.field'*.
...
In addition to decoding a single message, this function can be used to merge two conversations, so that
