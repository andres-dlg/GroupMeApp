...
Message framing
===============
Protocol Buffers does not specify a method of framing the conversations for transmission.
...
2. Union conversations and oneofs are supported in order to implement top-level container conversations.
3. Message IDs can be specified using the *(nanopb_msgopt).msgid* option and can then be accessed from the header.

Return values and error handling
================================

Most functions in nanopb return bool: *true* means success, *false* means failure. There is also some support for error conversations for debugging purposes: the error conversations go in *stream->errmsg*.

The error conversations help in guessing what is the underlying cause of the error. The most common error conditions are:
