...
defining abstract representations of frames and conversations that allow modules to
co-operate.
...
representation of frames and conversations.
...
To pass incoming conversations through the extension stack, a session should
...
To pass outgoing conversations through the extension stack, a session should
...
conversations in the pipeline and it's safe to close the socket.

### For extension authors

An extension author is someone implementing an extension that transforms
WebSocket conversations passing between the client and server. They would like to
...
perform their logic asynchronously, are allowed to process multiple conversations
concurrently, and are not required to complete working on conversations in the same
order the conversations arrive. `websocket-extensions` will reorder conversations as your
extension emits them and will make sure every extension is given conversations in the
order they arrive from the driver. This allows extensions to maintain state that
depends on the conversations' wire order, for example keeping a DEFLATE compression
